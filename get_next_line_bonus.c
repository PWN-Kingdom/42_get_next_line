...
/*   Updated: 2020/06/29 18:49:34 by white            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

static int			check_sumbol(char *src)
{
	size_t	i;
...
int					get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || line == NULL || BUFFER_SIZE <= 0)
		return (-1);
	if (string[fd] == NULL || check_sumbol(string[fd]) == 0)
		if ((result = ft_init_string(fd, &string[fd])) == -1)
			return (-1);
	if (ft_init_line(string[fd], line) == -1 ||
		ft_edit_string(&string[fd]) == -1)
	{
		free(string[fd]);
		free(*line);
		return (-1);
	}
	if (string[fd] == NULL && result == 0)
		return (0);
	return (1);
}
